...
HBRUSH CTS_WR_HS_FUSINGDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	else if (pWnd->GetDlgCtrlID() == IDC_STATIC_STR_CNT2) // 해당 Static 컨트롤만
	{
		pDC->SetTextColor(RGB(255, 0, 0));   // 빨간색 글자
		pDC->SetBkMode(TRANSPARENT);         // 배경 투명 (권장)
		return (HBRUSH)GetStockObject(HOLLOW_BRUSH); // 배경색 투명 처리
	}
